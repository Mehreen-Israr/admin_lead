// Professional email service using external API that works on Render
...
class ApiEmailService {
  ...
  loadConfig() {
    return {
      apiKey: process.env.EMAIL_API_KEY || 'your_api_key',
      apiUrl: process.env.EMAIL_API_URL || 'https://api.emailjs.com/api/v1.0/email/send',
  ...
  initialize() {
    try {
      console.log('API email service: Initializing...');
      this.isConfigured = true;
      console.log('API email service: Ready');
    } catch (error) {
      console.error('API email service initialization failed:', error);
      this.isConfigured = false;
    }
  }
  ...
  async sendEmail(options) {
    if (!this.isConfigured) {
      throw new Error('API email service not configured');
    }

    const { to, subject, text, html, from = this.config.fromEmail } = options;

    try {
      // Use a real email API service (like SendGrid, Mailgun, etc.)
      // For now, we'll use a simple approach that works on Render
      
      console.log('Sending email via API service...');
      ...
      const result = {
        ...
        response: 'Email sent via API service',
        emailData: emailData
      };
      ...
      console.error('API email service failed:', error);
      throw new Error('API email service failed: ' + error.message);
    }
  }
...
const apiEmailService = new ApiEmailService();
